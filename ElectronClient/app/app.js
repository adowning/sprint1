...
const {
  BaseApplication
} = require('lib/BaseApplication');
const {
  FoldersScreenUtils
} = require('lib/folders-screen-utils.js');
...
// const {
//   JoplinDatabase
// } = require('lib/joplin-database.js');
// const {
//   DatabaseDriverNode
// } = require('lib/database-driver-node.js');
const {
  ElectronAppWrapper
} = require('./ElectronAppWrapper');
const {
  defaultState
} = require('lib/reducer.js');
...
class Application extends BaseApplication {
  ...
  async generalMiddleware(store, next, action) {
    ...
    if (["NOTE_UPDATE_ONE", "NOTE_DELETE", "FOLDER_UPDATE_ONE", "FOLDER_DELETE"].indexOf(action.type) >= 0) {
      if (!await reg.syncTarget().syncStarted()) reg.scheduleSync(30 * 1000, {
  ...
  updateMenu(screen) {
              ...
              if (moduleSource === 'file') {
                path = bridge().showOpenDialog({
                  filters: [{
                    ...
                    extensions: module.fileExtensions
                  }]
                });
              } else {
                path = bridge().showOpenDialog({
    ...
    const template = [{
      ...
      submenu: [{
        ...
        accelerator: 'F1',
        click() {
          bridge().openExternal('https://joplin.cozic.net')
        }
      }, {
        label: _('Make a donation'),
        click() {
          bridge().openExternal('https://joplin.cozic.net/donate')
        }
      }, {
  ...
  async start(argv) {
    ...
    clipperLogger.addTarget('file', {
      path: Setting.value('profileDir') + '/log-clipper.txt'
    });
